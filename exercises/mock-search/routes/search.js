...
router.post('/', function(req, res) {
    ...
    search(req.body.key, data);
    res.send(results);

    function search(key, data, name) {
        for(var prop in data) {
            // if prop is not desc, call search again with prop 
            // pass the prop name as well to be used as a key
            if(prop !== 'desc') {
                search(key, data[prop], prop);
            }
            else {
                // desc found, search for the key
                if(data[prop].indexOf(key) !== -1) {
                    results.push({'name': name, 'desc': data[prop]});
                }
            }
        }
    }  
